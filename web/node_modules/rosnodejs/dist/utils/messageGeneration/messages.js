...
messages.getAll = function () {
  var msgLoader = new MessageLoader(false);
  return msgLoader.buildPackageTree(null, false).then(function () {
    var pkgCache = msgLoader.getCache();

    return new Promise(function (resolve, reject) {
      async.eachSeries(Object.keys(pkgCache), function (pkgName, pkgCallback) {
        async.eachSeries(['messages', 'services'], function (type, typeCallback) {
          var x = type;
          var msgs = pkgCache[pkgName][type];
          async.eachSeries(Object.keys(msgs), function (msgName, msgCallback) {
...
function buildMessageFromSpec(msgSpec) {
  var type = msgSpec.type;

  var fullMsg = msgSpec.getFullMessageName();
  switch (type) {
    ...
    case MessageSpec.SRV_TYPE:
      {
        var _ret = function () {
          var Request = buildMessageClass(msgSpec.request);
          var Response = buildMessageClass(msgSpec.response);
          var md5Sum = msgSpec.getMd5sum();
          var service = {
            Request: Request,
            Response: Response,
...
function buildMessageClass(msgSpec) {
  ...
  Message.Constants = function () {
    var ret = {};
    msgSpec.constants.forEach(function (constant) {
  ...
  var fullMsgDefinition = msgSpec.computeFullText();
...
function deserializeInnerMessage(spec, message, buffer, bufferOffset) {
  spec.fields.forEach(function (field) {
    ...
    if (field.isArray) {
      ...
      var ArrayMsgClass = void 0;
      var isPrimitive = field.isBuiltin;
      if (!isPrimitive) {

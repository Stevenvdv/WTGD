...
//varius handy dandy fuction for the atmel atmega 328 chip/Arduino
    ...
    void Antonius::sendByte(uint8_t address, uint8_t reg, uint8_t data)
    {

      uint8_t SLA_W = address<<1;
      uint8_t SLA_R=SLA_W|1;
      // Serial.println(SLA_W,HEX);
      // Serial.println(SLA_R,HEX);
      sendStart();
      TWDR = SLA_W;
      TWCR = (1<<TWINT) | (1<<TWEN);
      while (!(TWCR &(1<<TWINT)));
    ...
    uint8_t Antonius::requestByte(uint8_t address, uint8_t reg)
    {
      uint8_t SLA_W = address<<1;
      uint8_t SLA_R=SLA_W|1;
      uint8_t data;
    ...
    {
      uint8_t hour = requestByte(RTC_ADDRESS,HOUR);
    ...
    {
      uint8_t min = requestByte(RTC_ADDRESS,MIN);
      min = decodeMin(min);
      return min;
    }
    ...
    {
    //  Serial.println("blin");
      uint8_t sec = requestByte(RTC_ADDRESS,SEC);
    //  Serial.println("blin");
      sec = decodeSec(sec);
  //  Serial.println("blin");
      return sec;
